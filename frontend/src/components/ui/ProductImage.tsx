...
export default function ProductImage({
  ...
  // Reset loading and error states whenever src changes
  useEffect(() => {
    setIsLoaded(false);
    setHasError(false);
  }, [safeSrc]);

  // If no valid image URL exists or if loading failed: render neutral empty state
  if (!safeSrc || hasError) {
    return (
      <div
        className={cn(
  ...
  return (
    ...
    >
      {/* Neutral Shimmer Loading Skeleton (Visible while image is loading) */}
      {!isLoaded && (
        <div
