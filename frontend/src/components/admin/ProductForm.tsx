...
export default function ProductForm({ mode, productId, initialData }: ProductFormProps) {
  ...
  // 9. IMAGES (INITIALIZED TO EMPTY ARRAY - NO DEFAULT PLACEHOLDER)
  ...
  // Safe data normalization function for initializing form state
      ...
      api.getProduct(productId)
        .then((res) => {
          if (res && res.data) {
            ...
            // Check initialData fallback
            if (initialData) normalizeProductForForm(initialData);
          }
        })
        .catch((err) => {
          console.warn('Could not fetch product from backend API, attempting direct ID lookup:', err);
          api.getProductById(productId)
            .then((res) => {
              if (res && res.data) normalizeProductForForm(res.data);
              else if (initialData) normalizeProductForForm(initialData);
            })
            .catch((idErr) => {
              console.error('Failed to load product for editing:', idErr);
              if (initialData) {
                normalizeProductForForm(initialData);
              } else {
                setFetchError('Failed to load product from database. Please verify product ID or try again.');
              }
            });
        })
        .finally(() => {
  ...
  // Handle Category Change (Auto-clear subcategory for lounge-wear & kids-wear)
  ...
  // Client-side Image Compression Helper (reduces 5MB images to ~80KB)
  ...
  // Image Upload via API -> Cloudinary
  ...
  const handleSubmit = async (targetStatus: 'PUBLISHED' | 'DRAFT') => {
    ...
    // Format images (NO dummy placeholders)
  ...
  return (
    <div className="w-full space-y-6 pb-28 font-sans">
      ...
      {/* ── MAIN 2-COLUMN GRID LAYOUT (8 COLS LEFT, 4 COLS RIGHT) ── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* CARD 2: CATEGORY & CONDITIONAL SUBCATEGORY */}
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b pb-3">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              {/* Conditional Subcategory (Feeding / Non-Feeding) */}
              {hasFeedingSplit ? (
                <div>
                  <label className="text-xs font-bold text-gray-700 block mb-1">
          ...
          {/* CARD 3: PRICING & AUTOMATED DISCOUNT */}
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b pb-3">
              <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
          ...
          {/* CARD 4: PRODUCT VARIANTS & MATRIX */}
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b pb-3">
              <div>
                <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
