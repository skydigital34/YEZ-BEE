...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({
    email: 'patron@yezbeefashion.com',
    phone: '+91 98765 43210',
    firstName: 'Ananya',
    lastName: 'Singhania',
    address1: '42 Luxury Avenue, Bandra West',
    address2: 'Penthouse 12A',
    city: 'Mumbai',
    state: 'Maharashtra',
    pincode: '400050',
    country: 'India',
  ...
  const subtotal = totalAmount || 18999;
                  ...
                  <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[var(--color-champagne)]/60 shadow-soft-sm space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" value={form.firstName} onChange={(e) => updateField('firstName', e.target.value)} className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                      </div>
                      <div>
                        <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">Last Name</label>
                        <input type="text" value={form.lastName} onChange={(e) => updateField('lastName', e.target.value)} className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">Street Address</label>
                        <input type="text" value={form.address1} onChange={(e) => updateField('address1', e.target.value)} className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                      </div>
                      <div>
                        <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">City</label>
                        <input type="text" value={form.city} onChange={(e) => updateField('city', e.target.value)} className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                      </div>
                      <div>
                        <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">Pincode</label>
                        <input type="text" value={form.pincode} onChange={(e) => updateField('pincode', e.target.value)} className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[var(--color-champagne)]/60 shadow-soft-sm space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {([
                        ...
                        { id: 'upi', label: 'Instant UPI / GPay', icon: Smartphone },
                        ...
                        >
                          <m.icon size={20} className={paymentMethod === m.id ? 'text-[var(--color-primary-gold)]' : 'text-gray-400'} />
                    ...
                    {paymentMethod === 'card' && (
                      <div className="space-y-3 pt-4 border-t border-[var(--color-champagne)]">
                        <div>
                          <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">Card Number</label>
                          <input type="text" placeholder="4532 &bull;&bull;&bull;&bull; &bull;&bull;&bull;&bull; 8892" className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">Expiry Date</label>
                            <input type="text" placeholder="MM/YY" className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                          </div>
                          <div>
                            <label className="block text-xs font-bold uppercase text-[var(--color-dark)]/60 mb-1">CVV Security</label>
                            <input type="password" placeholder="&bull;&bull;&bull;" className="w-full text-xs font-semibold p-3 border border-gray-200 rounded-xl outline-none focus:border-[var(--color-primary-gold)]" />
                          </div>
                        </div>
                      </div>
                    )}
              ...
              {currentStep === 'review' && (
                <motion.div key="review" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {isSubmitting ? 'Securing Your Order...' : `Authorize & Place Order (₹${Math.round(finalTotal).toLocaleString()})`}
                    </button>
                  </div>
                </motion.div>
              )}
